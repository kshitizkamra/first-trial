...
db_sales_weekly=db_sales_final.groupby(['vendor style code','week']).agg({'total_orders':'sum','successful_orders':'sum','returns':'sum','customer_paid_amount':'sum','taxes':'sum','commission':'sum','logistics':'sum','settled_amount':'sum','cost':'sum'})
...
db_sales_weekly=db_sales_weekly.sort_values('successful_orders',ascending=False)
