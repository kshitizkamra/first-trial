...
total_pages=len(db_sales_summary)
...
if len(search_style_code)>0 :
   db_sales_summary_final=db_sales_summary[(db_sales_summary['vendor style code'].isin(search_style_code))]
    ...
    db_sales_summary_final=db_sales_summary
st.divider()
...
with col1 :
     st.image(db_sales_summary.loc[st.session_state['page_number'],'image link'], width=360)
     
     
with col2:
     db_sales_summary=db_sales_summary[['vendor style code','brand','article type','successful_orders','ROS','unit_customer_paid_amount','unit_cost','unit_p/l','roi','return %','val %','profit %','collection name','gender']].copy()
     db_sales_summary.rename(columns={'vendor style code': 'Style Code', 'brand': 'Brand','article type':'Category','successfull_orders':'Successfull Orders','unit_customer_paid_amount':'ASP','unit_cost':'COGS','unit_p/l':'Avg Profit per unit','roi':'ROI','return %':'Return','val %':'Revenue Contribution','profit %':'Profit Contribution','collection name':'Collection'}, inplace=True)
     st.table(data=db_sales_summary.iloc[st.session_state['page_number']-1])
...
brand=db_sales_summary.loc[st.session_state['page_number'],'Brand']
gender=db_sales_summary.loc[st.session_state['page_number'],'gender']
article_type =db_sales_summary.loc[st.session_state['page_number'],'Category']
...
if db_sales_summary.loc[st.session_state['page_number'],'ROS']>=db_checkaction_ros.a[0] :
    ros_action="A"
elif db_sales_summary.loc[st.session_state['page_number'],'ROS']>=db_checkaction_ros.b[0] :
    ros_action="B"
else :
    ros_action="C"
...
roi=db_sales_summary.loc[st.session_state['page_number'],'Avg Profit per unit']/db_sales_summary.loc[st.session_state['page_number'],'COGS']
...
returns=db_sales_summary.loc[st.session_state['page_number'],'Return']
...
with colm2 :
         ...
         with subsubcol1 :
             ...
             if st.button("✅ ", use_container_width=True):
                try :
                        style_code=db_sales_summary.loc[st.session_state['page_number'],'Style Code']
                        ...
                        insert_statement =action_items1.insert().values(selling_price=action_sp,vendor_style_code=db_sales_summary.loc[st.session_state['page_number'],'Style Code'])
                        insert_statement
